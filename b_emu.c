...
int _mcd(info_t *info)
{
	...
	if (!info->argv[1])
	{
		...
		if (!dir)
			chdir_ret = /* TODO: what should this be? */
				chdir((dir = _getenviro(info, "PWD=")) ? dir : "/");
		else
			chdir_ret = chdir(dir);
	}
	else if (_strcmp(info->argv[1], "-") == 0)
	{
		if (!_getenviro(info, "OLDPWD="))
		{
			_puts(s);
			_putchar('\n');
			return (1);
		}
		_puts(_getenviro(info, "OLDPWD=")), _putchar('\n');
		chdir_ret = /* TODO: what should this be? */
			chdir((dir = _getenviro(info, "OLDPWD=")) ? dir : "/");
	}
	else
	...
	{
		_setenviro(info, "OLDPWD", _getenviro(info, "PWD="));
