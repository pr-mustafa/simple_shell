...
/**
 * g_h_file - gets the history file
...
int r_history(info_t *info)
{
	...
	struct stat st;
	char *buf = NULL, *filename = g_h_file(info);
	...
	for (i = 0; i < fsize; i++)
		...
		{
			buf[i] = 0;
			b_history(info, buf + last, linecount++);
			last = i + 1;
		}
	if (last != i)
		b_history(info, buf + last, linecount++);
